/*******************************************************************************
* File Name: Upstream_485_IntClock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: Upstream_485_IntClock_Start
********************************************************************************
*
* Summary:
...
void Upstream_485_IntClock_Start(void) 
{
    ...
    Upstream_485_IntClock_CLKEN |= Upstream_485_IntClock_CLKEN_MASK;
	Upstream_485_IntClock_CLKSTBY |= Upstream_485_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Upstream_485_IntClock_Stop
********************************************************************************
*
* Summary:
...
void Upstream_485_IntClock_Stop(void) 
{
    ...
    Upstream_485_IntClock_CLKEN &= (uint8)(~Upstream_485_IntClock_CLKEN_MASK);
	Upstream_485_IntClock_CLKSTBY &= (uint8)(~Upstream_485_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Upstream_485_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Upstream_485_IntClock_CLKEN & Upstream_485_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Upstream_485_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Upstream_485_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Upstream_485_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Upstream_485_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Upstream_485_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Upstream_485_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Upstream_485_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Upstream_485_IntClock_GetSourceRegister();
    uint16 oldDivider = Upstream_485_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Upstream_485_IntClock_CLKEN & Upstream_485_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Upstream_485_IntClock_DIV_PTR, clkDivider);
                Upstream_485_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Upstream_485_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Upstream_485_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Upstream_485_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Upstream_485_IntClock_DIV_PTR, clkDivider);
				Upstream_485_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Upstream_485_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Upstream_485_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: Upstream_485_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Upstream_485_IntClock_MOD_SRC |= modeBitMask & (uint8)Upstream_485_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: Upstream_485_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Upstream_485_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Upstream_485_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: Upstream_485_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Upstream_485_IntClock_MOD_SRC & (uint8)(Upstream_485_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: Upstream_485_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Upstream_485_IntClock_GetDividerRegister();
    uint8 oldSrc = Upstream_485_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Upstream_485_IntClock_MOD_SRC |= CYCLK_SSS;
        Upstream_485_IntClock_MOD_SRC =
            (Upstream_485_IntClock_MOD_SRC & (uint8)(~Upstream_485_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Upstream_485_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Upstream_485_IntClock_MOD_SRC & Upstream_485_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Upstream_485_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Upstream_485_IntClock_PHASE = clkPhase & Upstream_485_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Upstream_485_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Upstream_485_IntClock_PHASE & Upstream_485_IntClock_PHASE_MASK;
}
